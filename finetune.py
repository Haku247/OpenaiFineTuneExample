...
def upload_training_dataset(output_jsonl_path):
    """Uploads the training dataset to OpenAI."""
...
def create_finetuning_job(uploaded_file_id):
    """Creates a fine-tuning job in OpenAI."""
...
def retrieve_finetuning_job(job_id):
    """Retrieves the status of a fine-tuning job from OpenAI."""
    response = openai.FineTuningJob.retrieve(job_id)
    print(response)
    print(f"Job {job_id} status: {response['status']}")


def ask(model, system_message, user_input):
    messages = [
        {"role": "system", "content": system_message},
...
def main():
    ...
    if operation == "generate_training_dataset":
        if len(sys.argv) < 4:
            print("Insufficient arguments for generate_training_dataset")
            sys.exit(1)
        generate_training_dataset(sys.argv[2], sys.argv[3])
    elif operation == "upload_training_dataset":
        if len(sys.argv) < 3:
            print("Insufficient arguments for upload_training_dataset")
            sys.exit(1)
        upload_training_dataset(sys.argv[2])
    elif operation == "create_finetuning_job":
        if len(sys.argv) < 3:
            print("Insufficient arguments for create_finetuning_job")
            sys.exit(1)
        create_finetuning_job(sys.argv[2])
    elif operation == "retrieve_finetuning_job":
        if len(sys.argv) < 3:
            print("Insufficient arguments for retrieve_finetuning_job")
            sys.exit(1)
        retrieve_finetuning_job(sys.argv[2])
    elif operation == "ask":
        if len(sys.argv) < 4:
            print("Insufficient arguments for ask")
            sys.exit(1)
        model = sys.argv[2]
